  ...
  <c-dialog ref="dialogForm" :title="`${title} ${Meta.title}`">
    <div class="max-w-[80vw] w-[1000px] bg-white relative rounded-md mx-auto">
      ...
      <form @submit.prevent="submit()">
        <div class="py-4 px-5 grid sm:grid-cols-3 gap-3">
          <div>
        ...
        <div>
          <c-table
            v-model:model-pagination="tablePermission.pagination"
            v-model:model-selection="form.permissions"
            :columns="columnsPermission"
            :data="tablePermission.data"
            :loading="tablePermission.loading"
            server-side
            select="id"
            without-shadow
            @changePage="getListPermission"
...
<script lang="ts">
  ...
  import * as MetaPermission from '../permission/meta';
  import { PermissionInstance } from '@/instances/permission.instance';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const $repoPermission = useHandler(MetaPermission.Meta);

      const { getList: getListPermission, table: tablePermission } = $repoPermission;
      const columnsPermission = MetaPermission.Meta.columns;
      ...
      const edit = async (id: number) => {
        const data: RoleInstance | null = await $repo.getData(id);
        if (data) {
          dialogForm.value?.show();
          resetForm();
          ...
          form.value = {
            id: data.id,
            name: data.name,
            permissions: data.permissions?.map((r: PermissionInstance) => r.id as number) || [],
          };
        }
      };

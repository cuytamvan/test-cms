  ...
  <c-dialog ref="dialogForm" :title="`${title} ${Meta.title}`">
    <div class="max-w-[80vw] w-[1000px] bg-white relative rounded-md mx-auto">
      ...
      <form @submit.prevent="submit()">
        <div class="py-4 px-5 grid grid-cols-3 gap-3">
          <div>
          ...
          <div>
            ...
            <c-input v-model="form.email" type="email" required />
          </div>

          <div>
            <label class="mb-1 block text-gray-700">Role <span class="text-red-500">*</span></label>

            <c-select
              v-model="form.role_id"
              :loading="select.role.loading"
              :options="select.role.data"
              option-label="name"
              option-value="id"
              searchable
            />
          ...
          <div>
            <label class="mb-1 block text-gray-700">Image</label>
            ...
            <img
              ...
              :src="form.image_url"
              :alt="form.name"
              class="w-full mt-5 p-5 border rounded-lg"
            />
          </div>
          ...
          <div class="col-span-2">
            ...
            <c-input v-model="form.password" required />
          </div>
        </div>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const getRole = async () => {
        const endpoint = `api/roles?_limit=0`;
        select.role.loading = true;
        const req = await $repo.http.get(endpoint);
        select.role.loading = false;

        if (req?.status === 200) select.role.data = req.data;
      };
      ...
      const chooseFile = () => {
        if (imageRef.value && imageRef.value.files) {
          form.value.image = imageRef.value.files[0] as File;
        }
      };
      ...
      const submit = async () => {
        const data = $repo.helper.setformdata(form.value, ['image']);

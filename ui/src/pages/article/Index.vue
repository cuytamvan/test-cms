<template>
  ...
  <c-dialog ref="dialogForm">
    <div class="max-w-[80vw] w-[1000px] bg-white relative rounded-md mx-auto">
      ...
      <form @submit.prevent="submit()">
        <img v-if="form.banner_url" :src="form.banner_url" :alt="form.title" class="w-full h-52 object-cover" />

        <div class="py-4 px-5 grid grid-cols-2 sm:grid-cols-3 gap-3">
          <div>
            <label class="mb-1 block text-gray-700">Title <span class="text-red-500">*</span></label>
            <c-input v-model="form.title" required />
          </div>

          <div>
            <label class="mb-1 block text-gray-700">Category <span class="text-red-500">*</span></label>

            <c-select
              v-model="form.article_category_id"
              :loading="select.articleCategories.loading"
              :options="select.articleCategories.data"
              option-label="name"
              option-value="id"
              searchable
            />
          ...
          <div class="overflow-hidden col-span-2 sm:col-span-1">
            <label class="mb-1 block text-gray-700">Banner</label>
            <input type="file" ref="imageRef" @change="chooseFile" />
          </div>

          <div class="col-span-3">
            <label class="mb-1 block text-gray-700">
              Content
              <span v-if="!form.id" class="text-red-500">*</span>
            </label>
            <c-editor v-model="form.content" class="w-full" />
          </div>
        </div>
  ...
  <c-section class="py-10">
    ...
    >
      ...
      <template #cell-role="{ raw }">
        <small
          v-if="raw.role"
          class="inline-block text-xs py-0.5 px-2 bg-blue-100 text-blue-700 rounded-full mb-1 mr-1"
        >
          {{ raw.artic.name }}
        </small>
      </template>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const defModel: ArticleInstance = {
        id: null,
      ...
      const form = ref<ArticleInstance>({
        ...defModel,
      });
      ...
      const getCategory = async () => {
        ...
        select.articleCategories.loading = true;
        const req = await $repo.http.get(endpoint);
        select.articleCategories.loading = false;

        if (req?.status === 200) select.articleCategories.data = req.data;
      };
      ...
      const chooseFile = () => {
        if (imageRef.value && imageRef.value.files) {
          form.value.banner = imageRef.value.files[0] as File;
        }
      };
      ...
      const edit = (data: ArticleInstance) => {
        dialogForm.value?.show();
        resetForm();
      ...
      const submit = async () => {
        const data = $repo.helper.setformdata(form.value, ['banner']);
        const req = form.value.id ? await $repo.updatePost(form.value.id, data) : await $repo.store(data);
